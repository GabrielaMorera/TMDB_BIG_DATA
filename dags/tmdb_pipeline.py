...
POSTGRES_PORT = "5433"  # Puerto actualizado
...
def process_kafka_data(**kwargs):
    ...
    try:
        ...
        output_file = f"{OUTPUT_DIR}/processed_movies.csv"
...
def load_to_postgres(**kwargs):
    ...
    if not processed_data_path or not os.path.exists(processed_data_path):
        ...
        processed_data_path = f"{OUTPUT_DIR}/processed_movies.csv"
    ...
    try:
        ...
        try:
            ...
            results_file = f"{OUTPUT_DIR}/results_data.csv"
...
def train_ml_model(**kwargs):
    ...
    try:
        ...
        model_dir = f"{OUTPUT_DIR}/ml_model_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
with DAG(
    ...
    description='Pipeline completo para datos de películas de TMDB',
    ...
    start_task = BashOperator(
        ...
        bash_command='echo "Iniciando pipeline de datos de TMDB" && mkdir -p {{ params.output_dir }}',
    ...
    end_task = BashOperator(
        ...
        bash_command='echo "Pipeline de datos de TMDB completado con éxito a las $(date)"'
    )
